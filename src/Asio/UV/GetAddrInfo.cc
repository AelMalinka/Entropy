...
GetAddrInfo::GetAddrInfo(Asio::MainLoop &loop, const function<void (struct addrinfo *)> &cb, const string &host, const string &service, int type)
	: _req(), _cb(cb), _hints()
{
	_req.data = this;
	...
	int ret = uv_getaddrinfo(dynamic_cast<MainLoop &>(loop).Handle(), &_req, _entropy_asio_uv_getaddrinfo_cb, host.data(), service.data(), &_hints);
	if(ret < 0)
		ENTROPY_THROW(
