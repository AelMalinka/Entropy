...
GetNameInfo::GetNameInfo(Asio::MainLoop &loop, const function<void (const string &, const string &)> &cb, const Stream &sock)
	: _req(), _cb(cb)
{
	_req.data = this;

	struct sockaddr s = sock.Socket();
	int ret = uv_getnameinfo(dynamic_cast<MainLoop &>(loop).Handle(), &_req, _entropy_asio_uv_getnameinfo_cb, &s, 0);
	if(ret < 0)
		ENTROPY_THROW(
